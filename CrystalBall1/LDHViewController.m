...
- (void)viewDidLoad
{
    ...
    /* background added by code
    UIImage *backgndImage = [UIImage imageNamed:@"background"];
    UIImageView *imageview = [[UIImageView alloc]initWithImage:backgndImage];
    [self.view insertSubview:imageview atIndex:0];
     */
    
    // find path of the sound file by accessing NSBundle
    NSString *soundPath = [[NSBundle mainBundle]pathForResource:@"crystal_ball" ofType:@"mp3"];
    // generated a URL with path
    NSURL *soundURL = [NSURL fileURLWithPath:soundPath];
    // a C function store soundid to system variable, since this is a C function & wont return anything so getting a reference to out pointer by using '&soundEffect'.
    AudioServicesCreateSystemSoundID(CFBridgingRetain(soundURL), &soundEffect);
    
    self.crystalBall = [[LDHCrystalBall alloc]init];
...
#pragma mark - Predicition

- (void)makePrediction {
    [self.backgroundImageView startAnimating];
    // playing the sound right after animating
    AudioServicesPlaySystemSound(soundEffect);
    self.predictionLabel.text = [self.crystalBall randomPredictions];
    self.predictionLabel.textColor = [UIColor purpleColor];
    self.predictionLabel.font = [UIFont fontWithName:@"Avenir Medium" size:10];
    //
...
- (void)motionBegan:(UIEventSubtype)motion withEvent:(UIEvent *)event {
    // need to check ion further while adding animations with this app.
